...
export excel "$tables/TabA2_Personality.xlsx", replace


** Self-Concept
use "$wdata/J1_for_analysis.dta", clear

factor cantsolve cantcontrol powerless valuable noproud optimal satisfy useless nothing
factor cantsolve cantcontrol powerless useless nothing valuable optimal satisfy //noproud factor loading < 0.5
rotate, blank(0.5)
matrix M = e(r_L)

alpha cantsolve cantcontrol powerless
local alpha1 = r(alpha)
alpha useless nothing
local alpha2 = r(alpha)
alpha valuable optimal satisfy
local alpha3 = r(alpha)

clear
set obs 11
gen item = ""
replace item = "Factor 1: Locus of Control" in 9
replace item = "Factor 2: Self-Efficacy" in 10
replace item = "Factor 3: Self-Esteem" in 11
replace item = "I can't solve some of my problems" in 1
replace item = "I can't control what happened to me" in 2
replace item = "I feel powerless" in 3
replace item = "I sometimes feel that I am useless" in 4
replace item = "Sometimes I think I am nothing" in 5
replace item = "I am a valuable/useful person" in 6
replace item = "I take optimal attitude on myself" in 7
replace item = "I am satisfy with myself" in 8

gen facload = .
forv i = 1(1)8{
	forv j = 1(1)3{
		replace facload = M[`i',`j'] if _n == `i' & abs(M[`i',`j']) > 0.5
	}
}


gen alpha = .
replace alpha = `alpha1' in 9
replace alpha = `alpha2' in 10
replace alpha = `alpha3' in 11

gen row = _n
recode row 9=0.5 10=3.5 11=5.5

sort row
drop row

format facload alpha %9.4f
export excel "$tables/TabA2_Self_Concept.xlsx", replace
