	...
	qui{
		...
		foreach x in female mainlander achieve{
			...
			foreach y in bct bct_ch bct_en bct_ma bct_sc bct_so {
				...
				cap parmest , saving("$tables/temp/FigureB2b_Random_Assign_Teacher_`x'_`y'.dta", replace) idstr("`x',`y'") idnum(`i')
			}
		}
		...
		if `i' == 1{
			...
			foreach x in female mainlander achieve{
				foreach y in bct bct_ch bct_en bct_ma bct_sc bct_so {
					ap using "$tables/temp/FigureB2b_Random_Assign_Teacher_`x'_`y'.dta"
				}
			}
			keep if strpos(parm,"favoritism") ~= 0
			save "$tables/temp/FigureB2b_Random_Assign_Teacher_.dta", replace
		}
		else{
			use "$tables/temp/FigureB2b_Random_Assign_Teacher_.dta", clear
			foreach x in female mainlander achieve{
				foreach y in bct bct_ch bct_en bct_ma bct_sc bct_so {
					ap using "$tables/temp/FigureB2b_Random_Assign_Teacher_`x'_`y'.dta"
				}
...
foreach x in female mainlander achieve{
	...
	foreach y in bct bct_ch bct_en bct_ma bct_sc bct_so {
		...
		parmest , saving("$tables/temp/FigureB2b_Random_Assign_Teacher_`x'_`y'.dta", replace) idstr("`x',`y'") idnum(0)
	}
...
keep if strpos(parm,"favoritism") ~= 0
save "$tables/temp/FigureB2b_Random_Assign_Teacher.dta", replace
